...
export class SmsCreateComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.companyService.getCompany().subscribe(
      (data) => {
        ...
        this.companyList.splice(4, 1)
        this.companyList.splice(1, 2)
  ...
  loadCompany() {
    this.idCompany = this.smsForm.get('company').value.id
    this.siteService.findSitesByCompany_Id(this.idCompany).subscribe(
      (data) => {
        this.siteList = data;
        this.smsForm.controls['site'].setValue("")
        this.smsForm.controls['line'].setValue("")
      }
    )
  }
  ...
  copyToClipboardCustomer(): void {
    // Create a temporary textarea element to hold the text
    const textarea = document.createElement('textarea');
    textarea.value = this.smsStringCustomer;
    document.body.appendChild(textarea);
    textarea.select();
    document.execCommand('copy');
    document.body.removeChild(textarea);
  ...
  filterSite() {
    this.siteService.findSitesByCompany_Id(this.idCompany).subscribe(
      data => {
        this.siteList = data
        let newSiteList = []
        let valueFilter = this.smsForm.get('valueFilter').value.toLowerCase()
